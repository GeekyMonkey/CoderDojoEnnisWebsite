...
export default defineEventHandler(
	async (event): Promise<AttendanceSignInResponseModel | AttendanceSignInErrorResponse> => {
		// Access the query string from the event object
		const query = event.node.req.url
			? new URL(event.node.req.url, `http://${event.node.req.headers.host}`).searchParams
			: new URLSearchParams();
		...
		try {
			...
			if (!member) {
				...
				return { error: `No member found with fingerprint ID ${fingerprintId}` };
			}

			// Do the sign-in
			const response: AttendanceSignInResponseModel = await attendanceService.signInMember(member, testing);
			return response;
		} catch (err) {
