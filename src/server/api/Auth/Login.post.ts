	...
	import { LoginRequest, loginRequestSchema } from "~~/shared/validation/AuthSchemas";
	...
	export default defineEventHandler(async (event): Promise<ApiResponse<ResponseBody>> => {
		...
		try {
			...
			const { member, session } = await authService.loginWithPassword(username, password, logs);
			...
			return { success: false, error: "[Login] POST error: " + (err?.message || "Unknown error"), logs };
		}
	});
