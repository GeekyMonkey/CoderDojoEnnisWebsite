...
import { memberFromRecord, type MemberModel } from "~~/shared/types/models/MemberModel";
...
export default defineEventHandler(async (event): Promise<ApiResponse<ResponseBody>> => {
	...
	try {
		...
		const auth = await LoginToSupabase(memberEntityLike, logs);
