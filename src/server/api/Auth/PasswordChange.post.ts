	...
	import { PasswordChangeRequest, passwordChangeRequestSchema } from "~~/shared/validation/AuthSchemas";
...
export default defineEventHandler(async (event): Promise<ApiResponse<ResponseBody>> => {
	...
	try {
		...
		if (!parsed.success) {
			...
			return { success: false, error: parsed.error.issues.map(i => i.message).join(", "), logs };
		}
		const { username, oldPassword, newPassword } = parsed.data;

		const authService = new AuthService(event);
		const { member } = await authService.validateCredentials(username, oldPassword, logs);
		const updated = await MembersData.SetMemberPasswordHash(event, member.id, newPassword.trim());
		if (!updated) {
		...
		return { success: false, error: "[PasswordChange] POST error: " + (err?.message || "Unknown error"), logs };
	}
});
