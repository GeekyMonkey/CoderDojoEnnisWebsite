...
<template>
  <div class="session-stats">
    <div class="row"><strong>Date:</strong> <span>{{ sessionDate }}</span></div>
    <div class="row"><strong>Signed In:</strong> <span>{{ memberCount }}</span></div>
    <div class="row"><strong>Total Sessions:</strong> <span>{{ SessionCount }}</span></div>
    <div class="row"><strong>Total Attendance:</strong> <span>{{ AttendanceTotal }}</span></div>
...
<style scoped>
.session-stats { border:1px solid #ccc; padding:8px; border-radius:6px; font-size:0.8rem; background:rgba(255,255,255,0.5); }
.row { line-height:1.1rem; }
.list { list-style:none; padding:0; margin:6px 0 0; max-height:160px; overflow:auto; font-family:monospace; font-size:0.7rem; }
.list li { line-height:1.1rem; }
.status { margin-top:4px; font-size:0.75rem; }
.status.error { color:#b30000; }
.status.empty { font-style:italic; color:#555; }
</style>
